...
async function run() {
  const userId = 'tester';
  const sessionKey = 'powers-clean-capture-' + Date.now();
  ...
  // Populate powers directly:
  // 1. Titan Strike (Deactivated)
  // 2. Elemental Arsenal (Active, slot: Cryo Freeze)
  console.log('2. Configuring powers in store...');
  await evalInTab(`
    (() => {
      ...
      // 2. Elemental Arsenal Array (Active, activeSlotId: alt_ice_freeze)
      window.store.addPower({
        id: 'pow_elemental',
  ...
  await evalInTab(`
    (() => {
      const cards = Array.from(document.querySelectorAll('.power-cascade-card'));
      if (cards.length > 0) {
  ...
  await evalInTab(`
    (() => {
      const cards = Array.from(document.querySelectorAll('.power-cascade-card'));
      if (cards.length > 1) {
        cards[1].scrollIntoView({ behavior: 'instant', block: 'center' });
      }
    })()
  `);
  await new Promise(r => setTimeout(r, 800));
  ...
  console.log('5. Capturing Targeted Effects...');
  await evalInTab(`
    (() => {
      const targetSec = document.getElementById('targeted-effects-container');
      if (targetSec) {
        targetSec.scrollIntoView({ behavior: 'instant', block: 'center' });
      }
    })()
  `);
  await new Promise(r => setTimeout(r, 800));
